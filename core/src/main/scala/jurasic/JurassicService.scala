...
/**
  * Jurassic Service simulating a microservice oriented Jurassic Park Service!
  *
  ...
  *
  *   `TRexClient.visit: Unit`, but is known to throw an exception when visitor dies
  ...
  *
  * - After a successful visit to the T-Rex the visitor gets a T-Shirt (which is a Souvenir)
  * - Our service will call `def visitAttractions(visitorId: String): Either[Error, List[Souvenir]]`
  *
  * Hints:
